...
int main()
{
    ...
    sf::VertexArray lines{ sf::LineStrip, 3 }; // crank shaft and piston rod
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            { sleep_time -= 5; if (sleep_time <= 15) sleep_time = 15; }
            else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down))
            { sleep_time += 5; if (sleep_time > 100) sleep_time = 100; }
        }
        ...
        window.draw(piston);
        window.draw(lines);
        window.draw(hline, 2, sf::LineStrip);
        window.display();
        sf::sleep(sf::milliseconds(sleep_time));
        lines[1].position.x = 100 * std::cos(theta) + 200;
        lines[1].position.y = -100 * std::sin(theta) + 200;
        lines[2].position.x = std::sqrtf(std::powf(150.0f, 2.0f) - std::powf(lines[2].position.y - lines[1].position.y, 2.0f)) + lines[1].position.x;
        piston.setPosition(lines[2].position.x, lines[2].position.y);
        theta += 0.05f;
